...
parser.add_argument("--env_name", default="MountainCarContinuous-v0")  # OpenAI gym environment name
...
parser.add_argument('--lr_alpha', default=5e-4, type=float)
parser.add_argument('--init_alpha', default=0.01, type=float)
...
parser.add_argument('--max_step', default=10000, type=int) # num of steps per gameparser.add_argument('--batch_size', default=128, type=int) # mini batch size
parser.add_argument('--seed', default=False, type=bool)
parser.add_argument('--random_seed', default=9527, type=int)
parser.add_argument('--batch_size', default=128, type=int)
parser.add_argument('--softmax_sampling', default=False, type=bool)
# optional parameters
...
class SoftmaxReplayBuffer:
    def __init__(self, capacity, alpha=0.01, device=device):
        self.buffer = []
        self.capacity = capacity
        self.alpha = alpha # Softmax Temperature
        self.dev = device
        self.base_prop = 0.2
    ...
    def sample(self, batch_size):
        ...
        exp_weights = np.exp(rewards / self.alpha)
        length = len(self.buffer)
        probabilities = (1.-self.base_prop) * exp_weights / np.sum(exp_weights) + self.base_prop / length

        chosen_indices = np.random.choice(range(length), size=batch_size, p=probabilities)
        samples = [self.buffer[i] for i in chosen_indices]

        states, actions, rewards, next_states, dones = map(np.array, zip(*samples))
        s_batch = torch.tensor(states, dtype=torch.float).to(self.dev)
        a_batch = torch.tensor(actions, dtype=torch.float).to(self.dev)
        r_batch = torch.tensor(rewards, dtype=torch.float).to(self.dev)
        s_prime_batch = torch.tensor(next_states, dtype=torch.float).to(self.dev)
        done_batch = torch.tensor(dones, dtype=torch.float).to(self.dev)
...
class SAC_Agent:
    def __init__(self, state_dim=state_dim, action_dim=action_dim, 
                 ...
                 tau=args.tau, init_alpha=args.init_alpha, lr_alpha=args.lr_alpha,
        ...
        self.memory = ReplayBuffer(capacity=capacity, device=self.DEVICE) if not softmax_sampling else SoftmaxReplayBuffer(capacity)
